...
async def add_image_to_exhibit(
    exhibit_id: int,
    ...
    try:
        ...
        new_image = await create_image(db, image_data)
        return new_image
...
async def update_exhibit_image(
    ...
    """
    # Check if image exists and belongs to the exhibit
    image = await get_image_by_id(db, image_id)
    if not image or image.exhibit_id != exhibit_id:
...
async def delete_exhibit_image(
    ...
    """
    Delete an image from an exhibit
    """
    # Check if image belongs to the exhibit
    image = await get_image_by_id(db, image_id)
    if not image or image.exhibit_id != exhibit_id:
        raise HTTPException(status_code=404, detail="Image not found for this exhibit")
