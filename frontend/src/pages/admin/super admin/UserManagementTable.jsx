                    ...
                    {filteredUsers.length > 0 && (
                        <div className="w-full bg-[#e6f0fa] py-2 px-4 flex justify-between items-center text-sm text-gray-700 font-medium rounded-b-lg border-t flex-shrink-0">
                            <span className="text-gray-600">
                                {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredUsers.length)} จาก {filteredUsers.length}
                            </span>
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                        <path d="M3.646 1.646a.5.5 0 0 1 .708 0l-6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L9.293 8 3.646 2.354a.5.5 0 0 1 0-.708"/>
                        ...
                        {currentUsers.map((user, index) => (
                            <div key={user.user_code || index} className="p-3 flex items-center relative">
                                ...
                                <div className="relative">
                                    <button 
                                        className="p-2" 
                                        onClick={() => toggleActionMenu(user.user_code)}
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
...
const UserManagementTable = () => {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            const res = await fetch(`${apiConfig.baseUrl}${API_PATH}/users/${user.id}`, { method: 'DELETE' });
            if (res.ok) {
                ...
                setPopupCountdown(5);
                setPopup({ open: true, type: 'fail', message: 'ลบผู้ใช้ไม่สำเร็จ', user: null });
            }
        } catch {
            setPopupCountdown(5);
            setPopup({ open: true, type: 'fail', message: 'ลบผู้ใช้ไม่สำเร็จ', user: null });
